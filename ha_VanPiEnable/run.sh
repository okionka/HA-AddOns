...
# all configs needs to be added to /config.txt
config='dtparam=i2c_vc=on'
config2='dtparam=i2c_arm=on'
config3='dtoverlay=w1-gpio'
config4='dtoverlay=uart5'
config5='enable_uart=1'
...
until false; do
  ...
  performWork () {
    ...
    if [ -e /tmp/$partition/config.txt ]; then
      ...
      if grep "$config3" /tmp/$partition/config.txt|grep -v \#; then
        echo "1-wire already configured on $partition. This addon was already run during this boot and no reboot occurred. ";
      else
        echo "adding $config3 to $partition config.txt"
        echo "$config3">>/tmp/$partition/config.txt
      fi
      if grep "$config4" /tmp/$partition/config.txt|grep -v \#; then
        echo "Serial already configured on $partition. This addon was already run during this boot and no reboot occurred. ";
      else
        echo "adding $config4 to $partition config.txt"
        echo "$config4">>/tmp/$partition/config.txt
      fi
      if grep "$config5" /tmp/$partition/config.txt|grep -v \#; then
        echo "Serial already enabled on $partition. This addon was already run during this boot and no reboot occurred. ";
      else
        echo "adding $config5 to $partition config.txt"
        echo "$config5">>/tmp/$partition/config.txt
      fi
    else
